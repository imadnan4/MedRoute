...
def _serializable_entry(entry: dict[str, Any]) -> tuple[dict[str, Any], bytes | None]:
    """Remove binary audio from JSON and return it as Postgres bytea."""
...
class NeonEncounterStore:
    ...
    def get(self, case_id: str, user_id: str) -> dict[str, Any] | None:
        ...
        return {
            "case_id": str(row[0]),
            "mode": row[1],
            "request": _json_object(row[2]),
            "stages": _json_object(row[3]),
            "result": _json_object(row[4]),
